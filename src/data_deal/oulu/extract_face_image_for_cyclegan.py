...
def get_input_face(image, rect, size):
    sx, sy, ex, ey = rect
    # h, w, c = image.shape
    # faceh = ey - sy
    # facew = ex - sx
    ...
    return cv2.resize(image[sy:ey, sx:ex, :], (size, size))
...
def  process_paths(paths, args, lock, counter, total_length):
    ...
    for img_path in paths:
        ...
        if len(bbox) != 4 or sum(bbox) < 1:
            ...
            if args.level>1:
                image_name = os.path.join(out_dir, '_'.join(img_path.split(str(Path('/')))[-1*args.level:]))
            else:
                image_name = os.path.join(out_dir, os.path.basename(img_path))

            out_img_path = image_name

            sx, sy, ex, ey = bbox
            #face_img,fsx, fsy, fex, fey = get_input_face(image, bbox, args.out_imgsize)
            face_img = get_input_face(image, bbox, args.out_imgsize)
